...
4.  **Ожидание:** Сначала отфильтровываются неверные форматы, затем все прокси проверяются параллельно, и каждый результат приходит сразу же. Четыре плитки над полем ввода — прогресс, работает, лучший пинг, ошибки/пропущено — обновляются по ходу.
5.  **Сбор результатов:** Рабочие прокси появляются в таблице результатов, самые быстрые первыми — скопируйте строку, нажмите **«Копировать список»** или экспортируйте в TXT/JSON.
