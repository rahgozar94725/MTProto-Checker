...
- **Two Go tests depend on a proxy list that is not in the repo.** `TestBatchParseAndTcpCheck` and `proxytest_test.go`'s `TestTelegramCheckWorkingProxies` read `testdata/proxies.txt` and `t.Skip` when it is absent. The other nine run on a fresh clone (`go test . -short -v`: 9 pass, 2 skip), so the earlier "largely a no-op" wording here was wrong. The JS suites have no such dependency — they run from a synthetic fixture.
- **The Go handlers themselves have no tests.** `TestReadCheckRequests` does use `httptest`, but only against the shared `readCheckRequests` helper — nothing builds the mux, so `/check`, `/check-batch` and `recoverMiddleware` have zero coverage. (An older entry here claimed no Go test used `httptest` at all; that has not been true for a while.) `checkProxy` is exercised only by a live-network test that skips when the local proxy list is absent. `/check-stream` is the exception, and only from the outside: the Playwright specs drive the real server, though `scan.spec.js` intercepts the endpoint and only the `E2E_LIVE` smoke test lets a request through to it.
