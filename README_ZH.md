...
* **内置代理列表:** 点击 **"加载列表"** 即可获取每晚从 17 个公开来源重建的列表 — 网络可达时取最新副本，被拦截时使用二进制内置的副本。来源可添加或关闭，并按各自提供的可用代理数量排名，抓取还可经由 SOCKS5 代理。
...
3.  **或加载内置列表:** 点击 **"加载列表"** 获取由 17 个公开来源每晚构建的列表 — 先取网络副本，被拦截时改用二进制内置的副本。在 **"列表来源"** 中添加自己的网址或关闭来源；若这些列表本身被过滤，可在 **"SOCKS5 代理（可选）"** 中配置代理。
4.  **开始检查:** 点击 **"开始检查"** 按钮。
5.  **等待:** 先过滤无效格式，然后并发检查所有代理，每个结果一出来就立即回传。输入框上方的四个信息块 — 进度、可用、最佳延迟、失败/跳过 — 会随之实时更新。
6.  **收集结果:** 可用代理按延迟从快到慢显示在结果表中 — 可逐行复制、点击 **"复制可用列表"**，或导出为 TXT/JSON。
